...
async function decodeResponseBody(response) {
	...
	for (const decode of attempts) try {
...
function isDeflate(bytes) {
	return bytes.length >= 2 && bytes[0] === 120 && (bytes[1] === 1 || bytes[1] === 156 || bytes[1] === 218);
}
function isZstd(bytes) {
	if (bytes.length < 4) return false;
	if (bytes[0] === 40 && bytes[1] === 181 && bytes[2] === 47 && bytes[3] === 253) return true;
	const descriptor = bytes[3];
	return bytes[0] === 80 && bytes[1] === 42 && bytes[2] === 77 && descriptor >= 24 && descriptor <= 31;
}
